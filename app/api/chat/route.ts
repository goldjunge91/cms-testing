...
import { convertToCoreMessages, LanguageModelV1, streamText, tool } from "ai";
...
`;
	...
	// Verwende die korrekte OpenAI-Modellbezeichnung und stelle sicher, dass es mit LanguageModelV1 kompatibel ist
	const result = await streamText({
		// model: openai("gpt-4") as unknown as LanguageModelV1,
		model: openai("gpt-4") as unknown as LanguageModelV1,
		system,
		...
		onFinish: async ({ text, toolCalls, toolResults, finishReason, usage }) => {
			...
			// Speichere die Nachricht des Assistenten
