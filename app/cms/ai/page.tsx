...
export default function Ai() {
	...
	const baseDomain = process.env.NEXT_PUBLIC_BASE_DOMAIN?.replace(/^https?:\/\//, '') || '';
												...
												<div className="pt-[0.5rem]">
													{'result' in toolInvocation && (
														<div className='flex gap-1'>
															<Link href={`https://${toolInvocation?.result?.site_subdomain}.${baseDomain}`} target="_blank">
										...
										return (
											<div key={toolCallId} className='bg-blue-700 bg-opacity-10 text-sm whitespace-pre-wrap px-3 py-2 rounded-lg'>
												{toolInvocation?.result?.message && JSON?.parse(toolInvocation?.result?.result).map((info: any) => (
													<div key={info?.site_name}>
														{toolInvocation?.result?.message + " "}
														{info?.site_name}: <Link href={`https://${info?.site_subdomain}.${baseDomain}`} className="underline" target="_blank">{info?.site_subdomain + "." + baseDomain}</Link>
													</div>
												))}
											</div>
										);
